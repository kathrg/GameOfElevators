...
void Building::spawnPerson(Person newPerson){
    //num floors newPerson wants to travel
    int request = newPerson.getTargetFloor() - newPerson.getCurrentFloor();
    
    //adds person to their starting floor
...
void Building::update(Move move){
    int peopleToPickup[move.getNumPeopleToPickup()];
    
    if (move.isPassMove()) {
    }
    else if (move.isPickupMove()) {
        //copies list of people to pick up into new array
        move.copyListOfPeopleToPickup(peopleToPickup);
        
        //use new array to remove people from current floor
        floors[elevators[move.getElevatorId()].getCurrentFloor()].removePeople(peopleToPickup, move.getNumPeopleToPickup());
        
        //sets appropriate elevator to its target floor
