...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    while (gameFile >> input) {
        ...
        while (building.getTime() < k.getTurn()) {
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    const Floor& pickupFloor = building.getFloorByFloorNum(pickupFloorNum);
    const int numPeople = pickupFloor.getNumPeople();
    int maxTargetFloor = -1;
    
    // Check if pickupList length exceed capacity
    if (pickupList.size() > ELEVATOR_CAPACITY) {
       return false;
     }
    
    bool validIndices[ELEVATOR_CAPACITY] = {false};
    
    for (int i = 0; i < pickupList.size(); i++) {
        const char indexChar = pickupList[i];
        // Check if index is between '0' & '9'
       if (!isdigit(indexChar)) {
         return false;
       }
        
        // Check if index is out-of-range or duplicate
        const int index = indexChar - '0';
        if (index >= numPeople || validIndices[index]) {
          return false;
        }
        validIndices[index] = true;
        
        // Check if target floor of person at index < number of people on the pickup floor.
        const Person& person = pickupFloor.getPersonByIndex(index);
        const int targetFloor = person.getTargetFloor();
        if (targetFloor > maxTargetFloor) {
            maxTargetFloor = targetFloor;
        }
    }
        // Check person is going in same direction as elevator
        int direction = maxTargetFloor - pickupFloorNum;
        if (direction == 0) {
            return false;
        }
    
        for (int i = 0; i < pickupList.size(); i++) {
            const int index = pickupList[i] - '0';
            const int targetFloor = pickupFloor.getPersonByIndex(index).getTargetFloor();
            const int personDirection = targetFloor - pickupFloorNum;
            if (personDirection * direction < 0) {
                return false;
            }
        }
        return true;
